    ...
    public abstract class NetworkedUdonSharpBehaviour : UdonSharpBehaviour
    {
        ...
        public sealed override void OnPostSerialization(VRC.Udon.Common.SerializationResult result)
        {
            if (!result.success)
            {
                ...
                Debug.LogError($"[{name}] Serialization failed for {result.byteCount} bytes.");
                return;
            }
        ...
        public sealed override void OnDeserialization()
        {
            if (clock <= oldClock)
            {
                Debug.LogError($"[{name}] Rejecting network update: old ({oldClock}) not less than new ({clock})");
                return;
            }

            OnNetworkUpdate();
            
            Debug.Log($"[{name}] Setting old clock ({oldClock}) to new clock ({clock})");
            oldClock = clock;
        }
